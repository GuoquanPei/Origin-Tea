...
class MobileNetV3_Tea_s1s2_0MB(nn.Module):
    def __init__(self, num_classes=6, act=nn.Hardswish):
        super(MobileNetV3_Tea_s1s2_0MB, self).__init__()

        # 仅保留 s0，直接连接到 bneck
        self.s0 = self._make_layer(conv_3x3_bn, 3, 64, 2, (224 // 2, 224 // 2))

        self.bneck = nn.Sequential(
            # Block(5, 64, 96, 96, act, True, 2),  # bneck 直接连接 s0，输入通道改为 64
        ...
        self.conv2 = nn.Conv2d(64, 576, kernel_size=1, stride=1, padding=0, bias=False)
...
if __name__ == '__main__':
    ...
    model = net = MobileNetV3_Tea_s1s2_0MB(num_classes=6).to(device)
    # print(model)
