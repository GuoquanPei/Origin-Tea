...
class MobileNetV3_Tea_s1_0MB(nn.Module):
    def __init__(self, num_classes=6, act=nn.Hardswish):
        super(MobileNetV3_Tea_s1_0MB, self).__init__()

        # 移除 s1 层
        self.s0 = self._make_layer(conv_3x3_bn, 3, 64, 2, (224 // 2, 224 // 2))
        self.s2 = self._make_layer(MBConv, 64, 192, 3, (224 // 4, 224 // 4))  # 直接连接到 s2

        self.bneck = nn.Sequential(
            # Block(5, 192, 96, 96, act, True, 2),
            # Block(5, 40, 240, 40, act, True, 1),
            # Block(5, 40, 240, 96, act, True, 1),
        ...
        self.conv2 = nn.Conv2d(192, 576, kernel_size=1, stride=1, padding=0, bias=False)
...
if __name__ == '__main__':
    ...
    model = net = MobileNetV3_Tea_s1_0MB(num_classes=5).to(device)
    # print(model)
