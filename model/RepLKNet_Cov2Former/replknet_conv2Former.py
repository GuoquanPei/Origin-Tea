...
from Conv2Former import Block as Conv2FormerBlock
...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        self.out_indices = out_indices #None
        if use_sync_bn:
            enable_sync_bn()
    ...
    def forward_features(self, x):
        ...
        for stem_layer in self.stem[1:]:
            if self.use_checkpoint:
                ...
                x = stem_layer(x)
        C0 = x #获取stem的输出  (B，channels, H/4. W/4)

        if self.out_indices is None:
            x = self.stages[0](x)
            x = self.transitions[0](x)
            C1 = x #(B，channels, H/8. W/8)
            x = self.stages[1](x)
            x = self.transitions[1](x)
            C2 = x #(B，channels, H/16. W/16)
            x = self.stages[2](x)
            x = self.transitions[2](x)
            C3 = x #(B，channels, H/32. W/32)
            x = self.stages[3](x)
            C4 = x #(B，channels, H/32. W/32)
    ...
    def forward(self, x):
        C0,C1,C2,C3,C4,x = self.forward_features(x)

        if self.out_indices:
            return x
        else:
            #x = self.PFA(C0,C1,C2,C3,C4)
            x = self.norm(C0,C1,C2,C3,C4) #或者直接替换这个norm层的定义  (1024,7,7)这里要在norm之前把特征维度规整到这个数上
            x = self.avgpool(x)
            x = torch.flatten(x, 1)
            x = self.head(x)
            return x
...
if __name__ == '__main__':
    model = create_RepLKNet31B(small_kernel_merged=False, num_classes=37)
    model.eval()
